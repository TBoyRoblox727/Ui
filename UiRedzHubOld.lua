...
function redzlib:MakeWindow(Configs)
	...
	function Window:Notify(Configs)
        ...
        local ContentText = Configs.Content or "Loading..."
        local Image = Configs.Image or "rbxassetid://0"
        local Duration = Configs.Duration or 5

        local NotifyGui = game:GetService("CoreGui"):FindFirstChild("RedzNotifySystem")
        if not NotifyGui then
            NotifyGui = Create("ScreenGui", game:GetService("CoreGui"), {Name = "RedzNotifySystem"})
        end

        local NotifyHolder = NotifyGui:FindFirstChild("NotifyHolder")
        if not NotifyHolder then
            NotifyHolder = Create("Frame", NotifyGui, {
                Name = "NotifyHolder",
                Size = UDim2.new(0, 240, 1, -40), -- Thu hẹp holder lại
        ...
        -- Khung siêu nhỏ gọn (Compact), Nền tối đậm
        local NotifyFrame = InsertTheme(Create("Frame", NotifyHolder, {
            Size = UDim2.new(0, 230, 0, 48), -- Nhỏ lại tối đa
            BackgroundColor3 = Color3.fromRGB(15, 15, 15), -- Ép nền tối đậm cho sang
            BorderSizePixel = 0,
            ClipsDescendants = true,
            Transparency = 0
        }), "Frame")
        Make("Corner", NotifyFrame, UDim.new(0, 6))
        Make("Stroke", NotifyFrame, {Color = Color3.fromRGB(45, 45, 45), Thickness = 1})

        -- Icon (Nhỏ lại cho hợp khung)
        local Icon = Create("ImageLabel", NotifyFrame, {
        ...
        -- Title (Trắng đậm)
        local LTitle = Create("TextLabel", NotifyFrame, {
        ...
        -- Content (Trắng thường)
        local LContent = Create("TextLabel", NotifyFrame, {
        ...
        -- Bộ đếm tíc tắc (Trắng, góc trên phải)
        local TimeLabel = Create("TextLabel", NotifyFrame, {
            Size = UDim2.new(0, 30, 0, 18),
            Position = UDim2.new(1, -48, 0, 6),
            Font = Enum.Font.GothamBold,
            TextSize = 11,
            Text = string.format("%.1fs", Duration), -- Hiển thị 5.0s
        ...
        -- Nút đóng (x)
        local CloseBtn = Create("TextButton", NotifyFrame, {
        ...
        local function CloseNotify()
            ...
            isClosing = true
            local out = TS:Create(NotifyFrame, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
        ...
        CloseBtn.Activated:Connect(CloseNotify)

        -- Tween vào
        NotifyFrame.Position = UDim2.new(1.5, 0, 0, 0)
        TS:Create(NotifyFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        ...
        task.spawn(function()
            local start = tick()
            while tick() - start < Duration and not isClosing do
                local remain = math.max(0, Duration - (tick() - start))
                TimeLabel.Text = string.format("%.1fs", remain)
                task.wait(0.1)
            end
            if not isClosing then CloseNotify() end
        end)
    end
    ...
    local Title = Configs.Title or Configs.Name or "Discord"
    local Desc = Configs.Desc or Configs.Description or ""
    local Logo = Configs.Logo or ""
    local Banner = Configs.Banner or ""
    ...
    local Members = FormatNum(Configs.Members or 0)
    local Online = FormatNum(Configs.Online or 0)
    
    -- Tăng chiều cao lên 160 để Description có chỗ thở
    ...
    Make("Stroke", FrameHolder, {Color = Theme["Color Stroke"], Thickness = 1.2})

    -- 1. Banner (Dùng Scale để luôn rộng hết cỡ)
    local BannerImg = Create("ImageLabel", FrameHolder, {
        Size = UDim2.new(1, 0, 0.4, 0), -- Chiếm 40% chiều cao khung
        Image = Banner,
    ...
    -- 2. Logo (Cố định kích thước nhưng vị trí linh hoạt)
    local ImageLabel = Create("ImageLabel", FrameHolder, {
        Size = UDim2.new(0, 45, 0, 45),
        Position = UDim2.new(0, 12, 0.25, 0), -- Nhảy theo chiều cao Banner
        Image = Logo,
        ZIndex = 10
    })
    Make("Corner", ImageLabel, UDim.new(0, 10))
    Make("Stroke", ImageLabel, {Color = Theme["Color Stroke"], Thickness = 2})
    
    -- 3. Title (Dùng Scale để tự dãn khi kéo UI sang ngang)
    local LTitle = InsertTheme(Create("TextLabel", FrameHolder, {
        Size = UDim2.new(1, -75, 0, 20),
        Position = UDim2.new(0, 65, 0.42, 0), -- Nằm ngay dưới Banner
        Font = Enum.Font.GothamBold,
        TextColor3 = Theme["Color Text"],
        TextXAlignment = "Left",
        BackgroundTransparency = 1,
        ...
        Text = Title,
        ZIndex = 10,
        ClipsDescendants = true -- Không cho chữ tràn ra ngoài
    }), "Text")
    
    -- 4. Info (Online/Members)
    local InfoLabel = Create("TextLabel", FrameHolder, {
        Size = UDim2.new(1, -75, 0, 15),
        Position = UDim2.new(0, 65, 0.55, 0),
        Font = Enum.Font.GothamMedium,
        TextColor3 = Color3.fromRGB(200, 200, 200),
        TextXAlignment = "Left",
        BackgroundTransparency = 1,
        TextSize = 10,
        RichText = true,
        Text = string.format("<font color='#43b581'>●</font> %s Online  <font color='#b9bbbe'>●</font> %s Members", Online, Members),
        ZIndex = 10
    })

    -- 5. Description (Sửa lại Size để không bị che)
    local LDesc = InsertTheme(Create("TextLabel", FrameHolder, {
        Size = UDim2.new(1, -24, 0, 35), -- Tăng diện tích hiển thị
        Position = UDim2.new(0, 12, 0.65, 5),
        TextWrapped = true,
        Font = Enum.Font.Gotham,
        TextColor3 = Theme["Color Dark Text"],
        TextXAlignment = "Left",
    ...
    JoinButton.Activated:Connect(function()
        ...
        local oldText = JoinButton.Text
        JoinButton.Text = "Copied link!"
        task.wait(2)
        JoinButton.Text = oldText
    end)
    
    return {
        Destroy = function() InviteHolder:Destroy() end
    }
end
		return Tab
	end
